...
**Important:** Gemini MCP has no conversation history. Every call is independent. The orchestrator must pass all relevant context (CODE_CONTEXT, EXTERNAL_CONTEXT, previous plans) in each prompt. The planner must return the full plan for each request.
